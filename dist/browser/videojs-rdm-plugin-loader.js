 ...
 * Version 2.97
 * 
 * Maintained by: Rogers Digital Media Team
 * 
 * Adds additional configuration and stuff to other Brightcove JS 
 * plugins used in tandem with the WP rdm-video-cloud plugin.  
 * 
 * 
 ******************** CODE BEGIN - RDM Plugin Loader *****************/
...
var pluginFunctions = {
  ...
  setup_omniture: function setup_omniture(player, options) {
    ...
    /* IMPORTANT! MUST use https:// since we're moving sites onto HTTPS 
     * otherwise, Adobe heartbeat code will assume http:// on servernames.  
     */
...
var initPlugin = function initPlugin(player, plugins) {
  ...
  if (other_events.length > 0) {
    var _loop = function _loop(_key2) {
      var func = other_events[_key2].func;
      var options = other_events[_key2].options;
      player.one(options.loading_event, function () {
        pluginFunctions[func](player, options);
      });
    };

    for (var _key2 in other_events) {
      _loop(_key2);
    }
  }
  ...
  if (ready_events.length > 0) {
    player.ready(function () {
      for (var _key3 in ready_events) {
        var _func = ready_events[_key3].func;
        var _options = ready_events[_key3].options;
        pluginFunctions[_func](player, _options);
      }
    });
  }
