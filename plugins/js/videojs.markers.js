...
(function($, videojs, undefined) {
   ...
   function registerVideoJsMarkersPlugin(options) {
      ...
      function getPosition(marker){
         var percent = setting.markerTip.time(marker) / player.duration() || 0;
         percent = (percent >= 1 ? 1:percent) * 100;
         var position = Math.round(percent*100)/100;
         return (position == 100) ? 99.5 : position;
      }

      function createMarkerDiv(marker, duration) {
         var markerDiv = $("<div class='"+setting.className+"'></div>")
         markerDiv.css(setting.markerStyle)
            .css({"margin-left" : parseFloat(markerDiv.css("width"))/2 + 'px',
               "left" : getPosition(marker) + '%',
               "height" : '8px', //this needs to be what vjs-load-progress is set to when user is active
               "top" : ($('.vjs-progress-holder').css('padding-top')) })
      ...
      function initialize() {
         ...
         //detach ads events
         player.off('ads-ad-started', player.markers.hide);
         player.off('ads-ad-ended', player.markers.show);
         ...
         for (var i = 0; i < cues.length; i++) {
            var marker = {time: cues[i].startTime, text: cues[i].text};
            markers.push(marker);
         }
         ...
         //attach ads event
         player.on('ads-ad-started', player.markers.hide);
         player.on('ads-ad-ended', player.markers.show);

      }


      // Since the plugin is loaded after the player is
      // initialized, loadedmetadata wont fire. hence we check to see current time is > 1 if it is, we can assume
      // loadedmetadata has fired and initialize the markers.
      player.on("loadedmetadata", initialize);

      // exposed plugin API
      player.markers = {
         hide: function(){
            $("."+setting.className).addClass('vjs-hidden');
         },
         show: function(){
            $("."+setting.className).removeClass('vjs-hidden');
         },
         getMarkers: function() {
           return markersList;
         },
