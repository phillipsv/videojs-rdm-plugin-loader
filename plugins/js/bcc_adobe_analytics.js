...
(function(ADB) {
	...
	VideoPlayerDelegate.prototype.getVideoInfo = function() {
		...
		if (this._player.mediainfo.duration < 0) { // Detecting live video
	...
	VideoPlayerDelegate.prototype.getQoSInfo = function() {
		...
		// Breaks iPhone content playback after ad
		// if (typeof this._player.tech.hls.playlists !== "undefined") {
		// 	currentRendition = this._player.tech.hls.playlists.media();
...
(function(ADB, VideoPlayerDelegate) {
	...
	function VideoAnalyticsProvider(appMeasurement, player, settings, isAutoplaying, debugFunction) {
		...
		ahPluginConfig.sdk = settings.HEARTBEAT.SDK;
                ahPluginConfig.ssl = settings.HEARTBEAT.SSL;
	...
	VideoAnalyticsProvider.prototype._setCustomMetadata = function() {
		...
		if (typeof this._player === 'undefined') {
			console.log("No player object to report custom meta");
			return;
		}

		if (typeof this._player.mediainfo === 'undefined') {
			console.log("No player mediainfo to report custom meta");
			return;
		}
		...
		if (this._player.mediainfo.custom_fields) {
			var customFields = this._player.mediainfo.custom_fields;
			for (var key in customFields) {
				customFieldsKeys.push(key);
				customFieldsValues.push(customFields[key]);
			}
		}

		customFieldsKeys = customFieldsKeys.join("|");
		customFieldsValues = customFieldsValues.join("|");

		// Filtering all tags to remove src=xxx
		for (var i=0; i < this._player.mediainfo.tags.length; i++) {
			if (typeof this._player.mediainfo.tags[i] !== "undefined") {
				if (this._player.mediainfo.tags[i].indexOf("src=") >= 0) {
					this._player.mediainfo.tags.splice(i, 1);
					i--;
				} else if (this._player.mediainfo.tags[i].indexOf("=") >= 0) {
					var array = this._player.mediainfo.tags[i].split("=");
					this._player.mediainfo.tags[i] = array[1];
				}
			}
		}
		...
		var eVars = {
			eVar41: this._player.mediainfo.id,
			eVar51: this._player.mediainfo.name,
			eVar53: tags.join()
		};

		if (window.self !== window.top && typeof parent._platform !== "undefined") {
			eVars.eVar4 = parent._platform;
		} else if (typeof _platform !== "undefined") {
			eVars.eVar4 = _platform;
		}

		if (customFieldsKeys != "" && customFieldsValues != "") {
			eVars.eVar54 = customFieldsKeys + "," + customFieldsValues;
		}
		...
		if (rdmDtm && !error) {
			if (rdmDtm.brand) {
				eVars.eVar2 = rdmDtm.brand;
			}

			if (rdmDtm.section) {
				eVars.eVar6 = rdmDtm.section;
			}
		}

		if (typeof rdmGrid !== 'undefined' && typeof rdmGrid.Browser !== 'undefined' && !error) {
			eVars.eVar10 = "na";
		} else if (rdmDtm.contentName) {
			eVars.eVar10 = rdmDtm.contentName;
		}

		this._aaPlugin.setVideoMetadata( eVars );
	};
	...
	VideoAnalyticsProvider.prototype._onComplete = function(e) {
		if (this._videoHeartbeat) {
			...
			this._aaPlugin.setVideoMetadata(null);
	...
	VideoAnalyticsProvider.prototype._setupVideoHeartbeat = function() {
		...
		if (!this._settings.HEARTBEAT.DISABLE) {
			...
			configData.channel = this._settings.HEARTBEAT.CHANNEL;
                        configData.ssl = this._settings.HEARTBEAT.SSL; 		
...
(function(window, document, vjs, undefined) {
	...
	defaults = {
			...
			VISITOR_API : {
				...
				TRACKING_SERVER : 'om.rogersmedia.com'
			},
			APP_MEASUREMENT : {
				RSID : 'rogersrmipublishing',
				TRACKING_SERVER : 'om.rogersmedia.com'
			},
			HEARTBEAT : {
			  DISABLE : false,
				...
				PUBLISHER : 'D7FD34FA53D63B860A490D44@AdobeOrg', // Set to your Adobe assigned publisher ID, set '' if not use
